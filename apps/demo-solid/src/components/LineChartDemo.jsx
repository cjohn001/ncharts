...
export const LineChartDemo = (props) => {
  let lineChartRef;
  const [currentStyle, setCurrentStyle] = createSignal('sales');
  ...
  const onStyleChange = (args) => {
    ...
    const styles = ['sales', 'stocks', 'weather'];
    setSelectedData(null);
    setCurrentStyle(styles[index]);
  };

  const styleIndex = createMemo(() => ['sales', 'stocks', 'weather'].indexOf(currentStyle()));
    ...
    <page>
      ...
      <gridlayout rows="auto, *, auto, auto" class="bg-slate-50 dark:bg-slate-900">
        ...
        <gridlayout row="0" columns="*, auto" class="p-4 pb-0">
          ...
          {selectedData() && (
            <gridlayout col="1" class="w-8 h-8 rounded-full bg-blue-500" on:tap={() => setSelectedData(null)}>
