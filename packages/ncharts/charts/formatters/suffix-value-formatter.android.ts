...
export class NSSuffixValueFormatter extends com.github.mikephil.charting.formatter.ValueFormatter {
  ...
  private static createFormatter(parsed: { integerDigits: number; fractionDigits: number; suffix: string }): java.text.DecimalFormat {
    const symbols = new java.text.DecimalFormatSymbols(java.util.Locale.getDefault());
    const formatter = new java.text.DecimalFormat('', symbols);

    formatter.setGroupingUsed(false);
    formatter.setMinimumIntegerDigits(parsed.integerDigits);
    formatter.setMinimumFractionDigits(parsed.fractionDigits);
    formatter.setMaximumFractionDigits(parsed.fractionDigits);
